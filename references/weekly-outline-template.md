...
本分支生成的是“每日任务”周大纲。周大纲不是标准课程目录，而是零基础学习者每天能照着做的任务导航。目录名推荐使用 `weekly-outlines/`，每日任务文件推荐放入 `daily-tasks/`。
...
- `# 📅 Week N：本周主题`
- `## 1. 本周方向`：写清本周要解决什么问题、为什么对 8 周路线重要、和 `AI-Interview` 的关系、本周最低可交付成果。
- `## 2. 本周新手门槛`：写本周最容易卡住的 3 个点、提前预警、暂时不要求掌握的高级内容。
- `## 3. Day 1-Day 7 每日任务`：每天必须写今日方向、今天只做三件事、卡点预警、验收标准、建议文件。
- `## 4. 本周核心产出物`：写目录结构、核心功能、关键代码、简历话术、面试可讲亮点。
- `## 5. 周末复盘`：写本周产出、仍不稳定的概念、下周前必须补的内容、README 或项目表达要补哪一段。
- `## 6. 避坑指南`：列出本周最容易浪费时间或走偏的点，尤其提醒不要做无法投递的孤立 Demo。

每日任务中的关联知识用稳定双链写在正文中，例如 `[[Python 虚拟环境]]`、`[[环境变量与 API Key]]`；不要把双链放入代码块。
